...
export const AdminShell: React.FC<AdminShellProps> = ({
  ...
  const [isHeaderEditionDropdownOpen, setIsHeaderEditionDropdownOpen] = useState(false);
  ...
  const regionDropdownRef = useDismissable<HTMLDivElement>(isRegionDropdownOpen, () => setIsRegionDropdownOpen(false));
  const editionDropdownRef = useDismissable<HTMLDivElement>(isEditionDropdownOpen, () => setIsEditionDropdownOpen(false));
  const headerEditionDropdownRef = useDismissable<HTMLDivElement>(isHeaderEditionDropdownOpen, () => setIsHeaderEditionDropdownOpen(false));
  ...
  return (
    <div className="flex h-dvh bg-bg text-text-primary overflow-hidden">
      ...
      <aside
        className={`fixed top-0 left-0 h-dvh bg-white border-r border-border flex flex-col z-50 transition-all duration-300 ${
          isMobileMenuOpen
        ...
        <div className={`p-3 border-b border-border space-y-2 select-none ${isMobileMenuOpen || !isDesktopCollapsed ? 'block' : 'hidden'}`}>
          ...
          <div className="relative" ref={regionDropdownRef}>
            <button 
              ...
              onClick={() => {
                ...
                setIsHeaderEditionDropdownOpen(false);
              }}
          ...
          {activeRegion && (
            <div className="relative" ref={editionDropdownRef}>
              <button 
                aria-haspopup="listbox"
                aria-expanded={isEditionDropdownOpen}
                onClick={() => {
      ...
      <main className={`flex-1 min-w-0 flex flex-col bg-bg overflow-hidden transition-all duration-300 ${
            ...
            <div className="flex items-center gap-1.5 sm:gap-2 text-xs font-bold text-text-primary min-w-0 flex-wrap">
              ...
              {activeRegion && (
                <>
                  ...
                  <Badge variant="info" size="sm" className="hidden sm:inline-flex text-[10px]">
                    {activeRegion.name}
                  </Badge>
                </>
              )}
              {activeRegion && (
                <>
                  <span className="text-text-tertiary">/</span>
                  <div className="relative" ref={headerEditionDropdownRef}>
                    <button 
                      aria-haspopup="listbox"
                      aria-expanded={isHeaderEditionDropdownOpen}
                      onClick={() => {
                        setIsHeaderEditionDropdownOpen(!isHeaderEditionDropdownOpen);
                        setIsRegionDropdownOpen(false);
                        setIsEditionDropdownOpen(false);
                      }}
        ...
        <div id="app-scroll" className="flex-1 overflow-y-auto">
          <div
            className="p-3.5 sm:p-5 md:p-6 max-w-7xl w-full mx-auto flex flex-col min-h-full"
      ...
      {isCreateModalOpen && (
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-xs flex items-center justify-center p-4 z-[9999] select-none text-left">
          <div role="dialog" aria-modal="true" className="bg-white border border-border rounded-lg max-w-md w-full p-5 shadow-modal animate-scale-up space-y-4">
