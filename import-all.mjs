...
const FORM_ID = 'bbc85f27-235c-4565-adcd-546a09d4c84e';

const EDITIONS = [
  { name: 'Kampala April 2026',   id: '5887fe74-41b8-4f84-a206-92877a6212a8', csv: '/Users/devosh/Downloads/Kampala April 2026.csv' },
  { name: 'Kampala May 2026',     id: 'b77818a3-ff36-42bc-986d-d91262c592de', csv: '/Users/devosh/Downloads/Kampala May 2026.csv' },
  { name: 'Kampala June 2026',    id: '3ce494f3-d462-474d-b32b-90b23447d261', csv: '/Users/devosh/Desktop/Quonnect-data-system-app/kampala_june_edition_2026.csv' },
  { name: 'Kampala August 2026',  id: '998fbf25-c888-49f0-9184-52507122f6bb', csv: '/Users/devosh/Downloads/Kampala_August_2026_survey_june-format.csv' },
];
...
const HEADER_MAP = {
  ...
  'gender': 'gender','sex': 'gender','age': 'age',
...
const SKIPPED_HEADERS = new Set([
  'start', 'end', '_id', '_uuid', 'submission_time', 'submitted_by', '__version__', 'root_uuid', '_index',
  'meta/rootuuid', '_status', '_notes', '_tags', 'validation_status', '_submission_time',
  '_submitted_by', 'kobo_id', 'kobo_uuid', 'row_index', 'form_version', 'submission_status',
]);
...
const QM = {
  ...
  business_category: 'b8aec37f-b458-411c-986c-cc79daa26c6d',
  how_long_in_business: '59a389d4-9322-4c4c-b9e6-fd1777048dc2', primary_source_of_income: 'a4bac9df-6ea3-4adf-ab92-f1b3d7d5cf81',
  products_primarily_from: '86f64dce-1630-48f5-b433-a1d60c620c8f', business_operates_as: 'd854b5be-1d42-4a0c-9e41-b04d8a107c3d',
  first_time_at_quonnect: '333f4198-8f90-46d2-bf6a-f64f723f6487', times_attended: '77b8b872-92f4-43a8-9e81-a148cecece75',
  attended_last_quonnect: '4ca12848-7b6d-429f-9a47-e2c19967d572', regions_attended: 'a201be01-6831-4e25-9994-2ceb65e236fe',
  paid_employees: 'c53c3db9-5599-403c-90ee-f840cf6df6f9', number_of_employees: '73f2e270-14fa-4677-9b00-bb7622fe6c0f',
  female_employees: '4a3596fe-4748-4ebc-af04-29d59f9b5bac', youth_employees: '0af0aa97-2420-441e-8f0d-f426c07cf1d8',
  hired_new_employees: '31f38046-92a1-4ef9-88ee-065977857ade', new_employees_count: 'ac19db83-a3c5-4f4e-96e3-493a6524e30f',
  hires_casual_helpers: '93cdfd0a-e35c-44ad-825e-3cfa8e11fc04', casual_helpers_count: '71c930be-f7ad-460c-954a-c89f5c5eef27',
  business_growth: '59742bda-5086-4300-bce1-a2a048cef780', quonnect_benefits: '9c95304c-e5ad-4283-b6c9-2e7d892cf6fb',
  active_social_media: '2ce847de-74e5-498d-bd96-937c6ce8dde4', online_sales: '26b0d5eb-869e-4707-8089-ccb9aa63a4bf',
  how_did_you_know: '25232bd6-08d7-4437-8596-10504e6bccbf', would_recommend: 'a9be34f3-a910-4326-955f-a0dca0495048',
  main_challenges: 'a0571885-3356-4771-b20a-706aba28eebd',
};
...
async function importEdition(edition) {
  const text = readFileSync(edition.csv, 'utf-8');
  const delimiter = text.split('\n')[0].includes(';') ? ';' : ',';
  const rows = parseCSV(text, delimiter);
  console.log(`\n${edition.name}: ${rows.length} rows`);

  let unmapped = new Set(), success = 0;

  for (let i = 0; i < rows.length; i++) {
    const clean = {};
    for (const [k, v] of Object.entries(rows[i])) { clean[cleanKey(k)] = String(v ?? '').trim(); }

    const normalized = {};
    const rowUnmapped = [];
    for (const [k, v] of Object.entries(clean)) {
      if (HEADER_MAP[k]) { normalized[HEADER_MAP[k]] = v; }
      else if (!SKIPPED_HEADERS.has(k) && !k.includes('/')) { rowUnmapped.push(k); unmapped.add(k); }
    }

    if (!normalized.full_name && !normalized.business_name) continue;

    const { data: v } = await supabase.from('vendors').upsert({
      contact_name: normalized.full_name || '', business_name: normalized.business_name || '',
      phone: normalized.phone_number || '', email: normalized.email || '',
      category: normalized.business_category || '', is_active: true,
    }, { onConflict: 'phone', ignoreDuplicates: false }).select('id').single();
    if (!v) continue;

    const { data: r } = await supabase.from('survey_responses').insert({
      form_id: FORM_ID, context_type: 'market_day', context_id: edition.id,
      vendor_id: v.id, source: 'csv_import',
      import_batch: edition.csv.split('/').pop(),
    }).select('id').single();
    if (!r) continue;

    const answers = [];
    for (const h of Object.keys(rows[i])) {
      const ck = cleanKey(h);
      const cc = HEADER_MAP[ck];
      if (!cc) continue;
      const qi = QM[cc];
      if (!qi) continue;
      const val = rows[i][h];
      if (!val) continue;
      answers.push({ response_id: r.id, question_id: qi, answer: String(val).trim() });
    }

    for (let j = 0; j < answers.length; j += 50) {
      await supabase.from('survey_answers').upsert(answers.slice(j, j + 50), {
        onConflict: 'response_id,question_id', ignoreDuplicates: true,
      });
    }
...
console.log('=== IMPORTING ALL 4 EDITIONS ===');
for (const ed of EDITIONS) { await importEdition(ed); }
console.log('\n=== COMPLETE ===');
